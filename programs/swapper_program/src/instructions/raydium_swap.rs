...
impl<'a, 'b, 'c, 'info> From<&mut UnifiedSwap<'info>>
    for CpiContext<'a, 'b, 'c, 'info, SwapBaseIn<'info>>
{
    fn from(accounts: &mut UnifiedSwap<'info>) -> CpiContext<'a, 'b, 'c, 'info, SwapBaseIn<'info>> {
...
pub fn swap_base_in(
    ctx: Context<'_, '_, '_, '_, UnifiedSwap<'_>>,
    amount_in: u64,
    ...
    let destination_account_data = &ctx.accounts.user_token_destination.data.borrow();
    let destination_account =
        TokenAccount::try_deserialize(&mut destination_account_data.as_ref())?;

    let balance_source_after = source_account.amount;
    let balance_destination_after = destination_account.amount;

    Ok((
        destination_account,
        balance_destination_after,
        source_account,
